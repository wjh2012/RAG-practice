"""Streaming LLM chain using OpenAI async client.

neo4j-graphrag's GraphRAG pipeline does not support SSE streaming,
so we use the OpenAI async client directly for streaming responses.
"""
...
2. 답변 시 출처(문서명)를 언급하세요.
3. 지식 그래프 관계 정보가 있으면 이를 활용하여 엔티티 간의 연결을 설명하세요.
4. 한국어로 답변하되, 고유명사나 기술 용어는 원문 그대로 사용하세요."""
